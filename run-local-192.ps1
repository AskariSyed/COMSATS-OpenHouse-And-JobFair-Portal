...
Write-Step "Local stack configuration"
...
if (-not $NoBackend) {
    ...
    Start-RepoProcess -Title "Backend (.NET)" -WorkingDirectory $backendDir -Command "dotnet run" -Dry:$DryRun
}
...
if (-not $NoAdmin) {
    ...
    $adminCommand = @(
        ...
        "npx vite --host=$LocalIp --port=5173"
    ) -join "; "
    Start-RepoProcess -Title "Admin Portal (Vite)" -WorkingDirectory $adminDir -Command $adminCommand -Dry:$DryRun
}

if (-not $NoCompany) {
    Write-Step "Starting Company Portal"
    $companyCommand = @(
        "`$env:VITE_SERVER_URL = '$backendUrl'",
        "npx vite --host=$LocalIp --port=5174"
    ) -join "; "
    Start-RepoProcess -Title "Company Portal (Vite)" -WorkingDirectory $companyDir -Command $companyCommand -Dry:$DryRun
}

if (-not $NoStudent) {
    Write-Step "Starting Student Portal"
    $studentCommand = @(
        "flutter pub get",
        "flutter run -d chrome --web-hostname $LocalIp --web-port 5175 --wasm --dart-define=BACKEND_BASE_URL=$backendUrl"
    ) -join "; "
    Start-RepoProcess -Title "Student Portal (Flutter Web)" -WorkingDirectory $studentDir -Command $studentCommand -Dry:$DryRun
}
...
if ($DryRun) {
    ...
    Write-Host "All selected services are launching in separate PowerShell windows." -ForegroundColor Green
